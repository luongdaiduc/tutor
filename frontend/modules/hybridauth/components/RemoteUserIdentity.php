...
class RemoteUserIdentity extends CBaseUserIdentity {
	...
	public function authenticate() {
		...
		if ($adapter->isUserConnected()) {
			...
			//set session for login FB/GG/TT user
			$profile = $this->_adapter->getUserProfile();

			$data = array('name'=>$profile->displayName, 'login_provider_id'=>$profile->identifier, 'provider'=>$this->loginProvider);
			app()->user->setState('socialNetworkName', $data);
