<?php $currency = !empty($invoice->currency) ? $invoice->currency : $setting['currency']?>
...
<table class="table table-striped table-hover table-condensed table-fluid">
	<tbody>
		<tr>
			<th style="width: 80%;">Upgrade to silver package</th>
			<td></td>
		</tr>
		<?php if($settings['gst_enable'] == 1) {?>
			<tr>
				<td style="width: 80%;"><?php echo $transaction->info?></td>
				<td style="text-align: right;"><?php echo Common::formatCurrency(number_format(($transaction->mc_gross/(1 + $settings['gst_rate']/100)), 2))?></td>
			</tr>
			<tr>
